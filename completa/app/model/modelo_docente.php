...
class modelodocente {
	...
	function mostrarnotificaciones() {
	    ...
	    //notificaciones de partes sin firmar (tutor)
	    $query = modelprinc::obtenerregistrosql('SELECT * FROM '.TABLAPARTES.' WHERE (firmatutor=0)AND(tutor='.$_SESSION["identificador"].')');
	    if ($query != "") {
	    	$return .= '<div id="advertencia"><h3>Atención: Tiene partes por firmar. '.contprinc::crearenlaceinterno("Pulse aqui para ver los partes por firmar.", "tutor", "miclase", '&clase='.modelotutor::obtenermitutoria($_SESSION["identificador"], "id").'').'</h3></div>';
	    }
	    //notificaciones de partes sin firmar (jefatura)
	    if (usuario::comprobarsisoy(4)) {
	    	$query = modelprinc::obtenerregistrosql('SELECT * FROM '.TABLAPARTES.' WHERE (firmajefatura=0)');
	    	if ($query != "") {
	    	$return .= '<div id="advertencia"><h3>Atención: Tiene partes por firmar. '.contprinc::crearenlaceinterno("Pulse aqui para ver los partes por firmar.", "jefatura", "listapartes").'</h3></div>';
	    	}
	    }
			return $return;
	}
